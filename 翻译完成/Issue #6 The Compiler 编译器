...
LLVM的指令非常棒，只要一个平台支持LLVM，那么LLVM就可以在这个平台上执行它的指令。比如说一个iOS的app，它就可以同时在完全不同架构的Intel和ARM平台上运行，所有的兼容性问题都是靠LLVM利用自己的中间表达码生成不同的原生机器码来解决不同平台的兼容性问题。

LLVM的优秀跨平台特性得益于其特定的“三层式”架构，即在第一层支持多种输入语言（比如：C,ObjectiveC,C++以及Haskell），第二层利用共享优化器来对LLVM中间表达码进行优化，第三层挂接了不同的平台（比如:Intel,ARM和PowerPC）。这样的话第一层和第三层之间做到了比较好的弱相关，如果想要增加对不同的输入语言的支持，只需要解决第一层的支持即可，要是想要增加目标编译平台，也不太需要操心输入语言的问题。如果对LLVM的架构感兴趣，可以参阅书籍The Architecture of Open Source Application，里面收录了由LLVM创造者Chris Lattner编写的介绍[LLVM架构][3]的章节。
...
本文重点关注阶段1和阶段2。在文[章Mach-O Executables][4]中，Daniel会对阶段3和阶段4进行阐述。
...
幸好现在有了[模块][5]引入特性，使得引用关系处理更加的智能。
...
鉴于本文的内容所限，就不对强大的预处理做更多、更全面的展开讨论了。但是还是要强调一点，建议大家不要在需要预处理的代码中加入内连联代码逻辑。
...
> 
> int main() {   int i = MyConstant;   printf("largest: %d\n",
> max(i++,100));   printf("i: %d\n", i);   return 0; }

这样改过之后，就可以输出正常的结果(i: 201)。因为之前定义的静态函数是直接插入在主体代码中的，所以他的效率和宏变量差不多，但是可靠性比宏定义要好许多。再者，还可以通过设断点debug、类型检查等手段来避免一些异常的产生。

基本上，宏的最佳使用场景是日志输出，可以使`用__FILE_`_ `或 __LINE_`_ 这种宏来做断言。
...
每当开发人员编写代码的时候，clang都会帮忙检查错误。其中最常见的就是检查是否对程序对象发送正确的消息，是否在数值上使用了正确的函数。比如对一个单纯的NSObject*对象发送了一个hello消息，clang就会报错。

又比如对NSObject创建一个Test子类：
...
一般会把类型们分类两类：动态的和静态的。动态的在运行时做检查，静态的在编译时做检查。以往，编写代码时可以向对象发送任何消息，因为在真正运行时，才会检查对象是否能够响应这些消息。鉴于只是在运行时做此类检查，所以叫做动态类型。至于静态类型，例如如果使用ARC，编译器会在编译过程中做许多检查，因为编译器需要确保对其编译的对象有明确的理解和认识。比如下面的代码中，如果myObject没有hello方法，这行代码就编不过。
...
看一下%9标注的那一行，发现这行代码正递归调用阶乘函数本身，这样是非常低效的，因为每次递归调用都要重新压栈。接下来可以看一下优化后的效果，在clang命令中增加-03标志：
...
接下来去使用者机器的LLVM目录下执行命令~/llvm/tools/clang/tools/，可以在这个目录下创建独立的clang工具。比如说，我们要创建一个小工具来帮助检查类库是否使用正确。把[样例工程][16]拷贝到这个目录下，然后执行make。接下来会生成一个叫example的二进制文件。
...
接下来，我们想要检查一下每当这个类被调用的时候，在目标对象中都有对应的响应action方法存在。可以写个C++函数来做这件事（注意，这是我第一次写C++程序，可能不那么专业）：
...
这个方法先是扫描消息方法的特定存在形式：以观察者作为消息方法的接收者，以observerWithTarget:action:作为selector，接着检查target中是否存在相应的selector。虽然这个例子有点儿刻意，但如果你想要利用AST对自己的代码库做某些机械检查，按照上面的例子来就可以了。
